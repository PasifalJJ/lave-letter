<template>
  <div style="block" :style="{width:animalWidth+'px',height:animalHeight+'px'}">
    <!-- 蓝鲸鱼 -->
    <canvas style="position: absolute;left:0px;top:0px" id="animal" ref="animal">Canvas not supported.</canvas>

    <el-col :xs="6" :sm="6" :md="4" :lg="4" :xl="4" class="wrapper"  style="position: absolute;right: 5%;bottom: 5%;">
      <div class="container">
        <div id="whale_container">
            ...
            <g>
              <g>
                <g id="Whale_group">
                  ...
                  <g id="whale"  @click=makeShower() ref="whale">
                    <path
                      id="whale_body"
...
export default {
  ...
  data() {
    return {
        animalWidth:window.innerWidth,
        animalHeight:window.innerHeight,
        animal:{},
        whale:{},
        eyeOpen:{},
        eyeClosed:{},
  ...
  mounted() {
    let animalDom = document.getElementById('animal');
    this.animal=this.$refs.animal;
    animalDom.width = window.innerWidth;
    animalDom.height = window.innerHeight;

    // animalDom.style.width = window.innerWidth + "px"
    // animalDom.style.height = window.innerHeight + "px"

    this.whale=this.$refs.whale;
    this.eyeOpen=this.$refs.eye_open;
    this.eyeClosed=this.$refs.eye_closed;
    this.waves = document.getElementsByClassName('.wave');
    this.sea=this.$refs.wavyLine;
    this.ctx=animalDom.getContext("2d");
    this.width = this.animal.width = window.innerWidth;
    this.height = this.animal.height = window.innerHeight;
  ...
  methods: {
    initParticles(){
        let _self = this;
        for (var i = 0; i < 100; i++) {
            setTimeout(this.createParticle(_self), 25 * i, i);
        }
    },

    createParticle(_self){
        //let x = _self.width * 0.5 - 17;
        //let y = _self.height * 0.5 - 70;
        let x = window.innerWidth /2;
        let y = window.innerHeight /3 ;
        let vx = -1.5 + Math.random() * 3;
        let vy = Math.random() * -8;
        let size = 4 + Math.random() * 3;
        let color = _self.colors[Math.floor(Math.random() * _self.colors.length)];
        let life = 7 + Math.random() * 9;
        ...
        let p = {
            update (i) {
                ...
                y += vy;
                life -= 0.2;
                if (_self.particles[i].remove() === true) {
            ...
            draw () {
                _self.ctx.beginPath();
                _self.ctx.globalAlpha = opacity;
                _self.ctx.fillStyle = color;
                _self.ctx.arc(x, y, size, 0, Math.PI * 2, false);
                _self.ctx.fill();
                _self.ctx.closePath();
            },
        }
        _self.particles.push(p);
    },

    animate(){
        this.ctx.clearRect(0, 0, this.animal.width, this.animal.height);
    ...
    addClasses(){
        this.whale.classList.add("shake");
        this.eyeOpen.classList.add("is-hidden");
        this.sea.classList.add("animate-sea");
    ...
    removeClasses(){
        this.whale.classList.remove("shake");
        this.eyeOpen.classList.remove("is-hidden");
        this.sea.classList.remove("animate-sea");
        this.waves.forEach(wave => {
    ...
    makeShower(){
        if(!this.isShowerOn){
            ...
            this.animate();
            this.addClasses();
            setTimeout(this.removeClasses,2000)
        }
    },

    onCanvaResize(){
          this.animalWidth=window.innerWidth;
          this.animalHeight=window.innerHeight;
